...
export default function DashboardPage() {
  ...
  const summaryStats = [
    { label: "Owned", value: owned.length, from: "from-sky-500", to: "to-teal-400"},
    { label: "Pre-booked", value: preBooked.length, from: "from-pink-500", to: "to-rose-400" },
    { label: "Final", value: final.length, from: "from-emerald-400", to: "to-teal-400"},
  ];
        ...
        <div className="flex-1 space-y-8">
          ...
          <Tabs defaultValue="owner" className="w-full">
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-xs text-gray-300">{l.city} • ${l.rent}/night</p>
                        </div>
            ...
            <TabsContent value="broker" className="space-y-3">
              {preBooked.length === 0 && (
                <p className="text-sm text-gray-400">No pre-booked properties yet.</p>
              )}
              {preBooked.map((b) => {
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-gray-300">
                        Pre-booked at ${b.price} {b.brokerPrice ? `• Re-list ${b.brokerPrice}` : ""}
                      </p>
                    </div>
                    <Link
                      className="text-sm font-medium underline hover:text-sky-400"
              ...
              {final.map((b) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <p className="font-medium">{p.title}</p>
                      <p className="text-sm text-gray-300">Access key available</p>
                    </div>
                    <Link
                      className="text-sm font-medium underline hover:text-sky-400"
                      href={`/nft/${p.id}`}
