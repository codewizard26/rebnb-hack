...
contract MarketplaceTest is Test {
    ...
    function testTokenURI() public view {
        assertEq(propertyToken.baseURI(), "https://base.uri/");
        assertEq(ITokenizedPropertyDate(propertyToken.date_token(propertyId)).baseURI(), "https://base.uri/1/");
        assertEq(propertyToken.tokenURI(propertyId), "https://base.uri/1");
    }
    ...
    function testCreateListing() public {
        ...
        assertEq(
            ITokenizedPropertyDate(propertyToken.date_token(propertyId)).tokenURI(date),
            string.concat("https://base.uri/1/", date.toString())
        );
        ...
        assertEq(listing.rentPrice, 1 ether, "Listing rent price is not 1 ether");
        assertEq(listing.rentSecurity, 0.1 ether, "Listing rent security is not 0.1 ether");
        assertEq(listing.bookingPrice, 2 ether, "Listing booking price is not 2 ether");
        assertEq(listing.bookingSecurity, 0.2 ether, "Listing booking security is not 0.2 ether");

        ITokenizedPropertyDate dateToken = ITokenizedPropertyDate(propertyToken.date_token(propertyId));
        assertEq(dateToken.ownerOf(date), owner, "Date token owner is not owner");
    }
    ...
    function testRentListing() public {
        ...
        vm.prank(owner);
        marketplace.createListing(propertyId, date, rentPrice, rentSecurity, bookingPrice, bookingSecurity);

        ITokenizedPropertyDate dateToken = ITokenizedPropertyDate(propertyToken.date_token(propertyId));
        assertEq(dateToken.ownerOf(date), owner, "Date token owner is not owner");
        // Prepare update params
        Marketplace.UpdateListingParams memory params = Marketplace.UpdateListingParams({
            rentPrice: 1 ether,
        ...
        assertEq(dateToken.ownerOf(date), renter, "Date token owner is not renter");
        assertEq(owner.balance, rentSecurity, "Owner balance is not rent security");
    }
    ...
    function testBookListing() public {
        uint256 date = block.timestamp / 86400 + 1;
        vm.prank(owner);
        marketplace.createListing(propertyId, date, 1 ether, 0.1 ether, 2 ether, 0.2 ether);
        ...
        assertEq(dateToken.ownerOf(date), booker, "Date token owner is not booker");
        assertEq(owner.balance, 0.2 ether, "Owner balance is not booking security");
    }

    function testBookListingAfterTwoRents() public {
        Marketplace.UpdateListingParams memory renter0params = Marketplace.UpdateListingParams({
            rentPrice: 1 ether,
            rentSecurity: 0.1 ether,
            bookingPrice: 2 ether,
            bookingSecurity: 0.2 ether
        ...
        Marketplace.UpdateListingParams memory renter1Params = Marketplace.UpdateListingParams({
        ...
        vm.prank(renter1);
        marketplace.rentListing{value: renter0params.rentSecurity}(propertyId, date, renter1Params);

        assertEq(owner.balance, renter0params.rentSecurity, "Owner balance is not rent security");
        assertEq(dateToken.ownerOf(date), renter1, "Date token owner is not renter1");
        _assertListing(propertyId, date, renter1Params, "Renter1:");

        // Second rent
        address renter2 = vm.randomAddress();
        vm.label(renter2, "Renter2");
        Marketplace.UpdateListingParams memory renter2Params = Marketplace.UpdateListingParams({
        ...
        vm.prank(renter2);
        marketplace.rentListing{value: renter1Params.rentSecurity}(propertyId, date, renter2Params);

        assertEq(renter1.balance, renter1Params.rentSecurity, "Renter1 balance is not rent security");

        assertEq(dateToken.ownerOf(date), renter2, "Date token owner is not renter2");
        ...
        vm.prank(booker);
        marketplace.bookListing{value: renter2Params.bookingSecurity}(propertyId, date);

        assertEq(renter2.balance, renter2Params.bookingSecurity, "Renter2 balance is not rent security");
    ...
    function _assertListing(
        ...
        Marketplace.Listing memory listing = marketplace.getListing(propertyId_, date_);
        assertEq(
            listing.rentPrice,
            params.rentPrice,
            string.concat(errorPrefix, "Listing rent price is not params.rentPrice")
        );
        assertEq(
            listing.rentSecurity,
            params.rentSecurity,
            string.concat(errorPrefix, "Listing rent security is not params.rentSecurity")
        );
        assertEq(
            listing.bookingPrice,
            params.bookingPrice,
            string.concat(errorPrefix, "Listing booking price is not params.bookingPrice")
        );
        assertEq(
            listing.bookingSecurity,
            params.bookingSecurity,
            string.concat(errorPrefix, "Listing booking security is not params.bookingSecurity")
        );
    }
}
